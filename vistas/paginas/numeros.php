              ...
              <div class="card-body p-0">
                <div class="table-responsive">
                  <table class="table m-0">
                    ...
                    <?php foreach ($numeros as $key => $value) : ?>
                        <?php 
                        $acciones = "<div class='btn-group'><button class='btn btn-primary shadow-none btn-sm editarNumero'data-bs-toggle='modal' data-bs-target='#editarNumeros' idNumero='" . $value["id"] . "'><i class='fas fa-pencil-alt shadow-none text-white'></i></div>";
                        // La clase .editarUsuario, me ayuda a obtener la informacion del usuario que quiero editar, esta clase la uso en usuarios.js
                        if($value["pasivo"] == 0){
                            $pasivo = "<button class='btn btn-success btn-sm btnActivar' idNumero='".$value['id']."' estadoNumero='1'>Activo</button>";
                        }else{
                            $pasivo = "<button class='btn btn-danger btn-sm btnActivar' idNumero='".$value['id']."' estadoNumero='0'>Inactivo</button>";
                        } ;
